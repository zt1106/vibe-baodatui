...
- **Game Engine (`packages/game-core`)** encapsulates pure poker state transitions (deck, seats).
- **Shared Contracts (`packages/shared`)** enforce schema parity between client and server via Zod.
- **In-memory Table Registry** tracks active seats and hands within each server process.
...
- Pure, deterministic functions for table lifecycle: `createTable`, `joinTable`, `deal`.
...
- Zod schemas for events (`JoinTable`, `ServerState`) to validate runtime payloads.
...
- Player seating and decks live in process-scoped maps keyed by table id.
...
- **Observability**:
  ...
  - Tracing (OpenTelemetry) for join flows to spot latency spikes.
...
- Role-based authorization for admin endpoints (table moderation, table management).
...
- **Unit Tests**: `pnpm -r test` runs Vitest suites across packages. Critical to cover game engine edge cases (deck exhaustion, duplicate joins).
- **Integration Tests**: Socket.IO client simulations (Vitest + `socket.io-client`) validate join flows end-to-end.
