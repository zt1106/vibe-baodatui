...
const CARD_INDEX_MAP = new Map();
BASE_CARDS.forEach((card, index) => CARD_INDEX_MAP.set(`${card.rank}${card.suit}`, index));
function resolveCardKey(rank, suit) {
  return `${rank}${suit}`;
}
function assertPackIndex(packIndex) {
...
export function createCardId(rank, suit, packIndex = 0) {
...
export function makeCard(rank, suit, faceUpOrOptions = false) {
